...
const IFRAME_FLASH_INTERVAL = 1.0 / 15.0
...
func update_transformation_availability():
	can_transform = (ScoreManager.rings >= super_ring_amount 
		and chaos_emeralds >= super_em_requirement 
		and !spun_sign_post 
		and !super_state)
...
func get_player_position():
	var y_offset = skin.transform.y * current_bounds.offset.y
	var x_offset = skin.transform.x * current_bounds.offset.x
	return skin.global_position + y_offset + x_offset
...
func handle_wall_collision():
	...
	wall_colliding_object = null
	
	if right_ray:
		wall_colliding_object = right_ray.collider
	...
	if left_ray:
		wall_colliding_object = left_ray.collider
...
func handle_ceiling_collision():
	...
	if !hits or velocity.y > 0:
		ceiling_colliding_object = null
		return
	
	ceiling_colliding_object = hits.closest_hit.collider
...
func handle_ground_collision():
	...
	if hits and velocity.y >= 0:
		ground_colliding_object = hits.closest_hit.collider
...
func handle_skin(delta):
	...
	if !__is_grounded:
		if skin_rotation_mode == "Original":
			skin.rotation_degrees = snapped(0, 45)  # Instant snap
		else:  # Smooth
			var current_rotation = skin.rotation_degrees
			skin.rotation_degrees = move_toward(current_rotation, 0, 360 * delta)
	else:
		if skin_rotation_mode == "Original":
			var target_angle = snapped(ground_angle, 45) if abs(ground_angle) > 10 else 0.0
			skin.rotation_degrees = target_angle
		else:  # Smooth
