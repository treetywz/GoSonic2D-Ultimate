...
func ending_cutscene():
	player.enable_cpu_input()
	set_player_limit_right(3412)
	player.change_state("Regular")
	player.cpu_move_player_right(true)
...
func custom_intro():
	...
	if Global.current_act == 1: # We will only run this custom intro on act 1.
		player.enable_cpu_input() # Enable the player's artifical inputs
		...
		player.cpu_move_player_right(true) # Make the player start moving right
		await pause(0.7)
		player.cpu_move_player_right(false) # Stop the player from moving right
		await pause(2)
		player.disable_cpu_input() # Give control back to the player
